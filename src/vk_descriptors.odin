...
init_pool :: proc(allocator: ^DescriptorAllocator, device: vk.Device, max_sets: u32, pool_ratios: []PoolSizeRatio, flags: vk.DescriptorPoolCreateFlags = {}) {
	...
	for ratio, i in pool_ratios {
		append(
			&pool_sizes,
			vk.DescriptorPoolSize{type = ratio.type, descriptorCount = u32(f32(ratio.ratio) * f32(max_sets))},
		)
	}
	...
	vk_check(vk.CreateDescriptorPool(device, &pool_info, nil, &allocator.pool))
}
