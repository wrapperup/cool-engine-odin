...
VulkanEngine :: struct {
	...
	mesh_compute_pipeline:          vk.Pipeline,
	...
	shared_compute_pipeline_layout: vk.PipelineLayout,
	voxel_buffer:                   AllocatedBuffer,
	voxel_buffer_address:           vk.DeviceAddress,
	draw_cmds_buffer:               AllocatedBuffer,
	draw_cmds_buffer_address:       vk.DeviceAddress,
...
create_swapchain :: proc(engine: ^VulkanEngine) {
	...
	rimg_info := init_image_create_info(engine.draw_image.format, draw_image_usages, draw_image_extent, ._4)
	...
	dimg_info := init_image_create_info(engine.depth_image.format, depth_image_usages, draw_image_extent, ._4)
...
init_pipelines :: proc(engine: ^VulkanEngine) {
	init_mesh_compute_pipelines(engine)
	init_mesh_pipelines(engine)
}

init_mesh_compute_pipelines :: proc(engine: ^VulkanEngine) {
	compute_shader, ok := util_load_shader_module("shaders/out/compute.spv", engine.device)
	if !ok {
		panic("compute fail!")
	}

	engine.voxel_buffer = create_buffer(
		engine,
		32*32*32,
	...
	engine.draw_cmds_buffer = create_buffer(
		engine,
		size_of(vk.DrawIndirectCommand),
		{.INDIRECT_BUFFER, .STORAGE_BUFFER, .TRANSFER_DST, .SHADER_DEVICE_ADDRESS},
		.GPU_ONLY,
	)

	voxel_addr_info := vk.BufferDeviceAddressInfo {
		sType  = .BUFFER_DEVICE_ADDRESS_INFO,
		buffer = engine.voxel_buffer.buffer,
	}

	draw_addr_info := vk.BufferDeviceAddressInfo {
		sType  = .BUFFER_DEVICE_ADDRESS_INFO,
		buffer = engine.draw_cmds_buffer.buffer,
	}

	engine.voxel_buffer_address = vk.GetBufferDeviceAddress(engine.device, &voxel_addr_info)
	engine.draw_cmds_buffer_address = vk.GetBufferDeviceAddress(engine.device, &draw_addr_info)
	...
	vk_check(
		vk.CreatePipelineLayout(engine.device, &pipeline_layout_info, nil, &engine.shared_compute_pipeline_layout),
	)

	pipeline_stage := vk.PipelineShaderStageCreateInfo {
		sType  = .PIPELINE_SHADER_STAGE_CREATE_INFO,
		module = compute_shader,
		pName  = "write_voxel_data",
		stage  = {.COMPUTE},
	}
	...
	pipeline_create_info := vk.ComputePipelineCreateInfo {
		...
		layout = engine.shared_compute_pipeline_layout,
	}

	vk_check(vk.CreateComputePipelines(engine.device, 0, 1, &pipeline_create_info, nil, &engine.mesh_compute_pipeline))

	draw_pipeline_stage := vk.PipelineShaderStageCreateInfo {
		sType  = .PIPELINE_SHADER_STAGE_CREATE_INFO,
		module = compute_shader,
		pName  = "write_draw_cmd",
		stage  = {.COMPUTE},
	}

	draw_pipeline_create_info := vk.ComputePipelineCreateInfo {
		sType  = .COMPUTE_PIPELINE_CREATE_INFO,
		stage  = draw_pipeline_stage,
		layout = engine.shared_compute_pipeline_layout,
	}

	vk_check(vk.CreateComputePipelines(engine.device, 0, 1, &draw_pipeline_create_info, nil, &engine.draw_pipeline))
}

init_mesh_pipelines :: proc(engine: ^VulkanEngine) {
	triangle_shader, f_ok := util_load_shader_module("shaders/out/shaders.spv", engine.device)
	...
	pb_set_multisampling(&pipeline_builder, ._4)
...
main_loop :: proc(engine: ^VulkanEngine) {
	...
	for (!glfw.WindowShouldClose(engine.window)) {
		...
		if (im.Begin("Camera")) {
			im.InputFloat3("pos", cast(^[3]f32)(&game_state.camera_pos))
			im.InputFloat3("target", cast(^[3]f32)(&game_state.camera_target))
			im.InputFloat("fov", cast(^f32)(&game_state.camera_fov_deg))
		}
		im.End()
...

		{
			...
			if glfw.GetKey(engine.window, glfw.KEY_SPACE) == glfw.PRESS {
				game_state.camera_pos += {0, 1, 0} * 0.05
			}
			if glfw.GetKey(engine.window, glfw.KEY_LEFT_SHIFT) == glfw.PRESS {
				game_state.camera_pos += {0, -1, 0} * 0.05
			}
		}
...
exec_voxel_compute :: proc(engine: ^VulkanEngine, cmd: vk.CommandBuffer) {
	vk.CmdBindPipeline(cmd, .COMPUTE, engine.mesh_compute_pipeline)

	push_constants := ComputePushConstants {
		voxel_buffer     = engine.voxel_buffer_address,
		draw_cmds_buffer = engine.draw_cmds_buffer_address,
		frame_time       = u32(engine.frame_number),
	}

	vk.CmdPushConstants(
		cmd,
		engine.shared_compute_pipeline_layout,
		{.COMPUTE},
		0,
	...
	vk.CmdBindPipeline(cmd, .COMPUTE, engine.draw_pipeline)
...
draw_geometry :: proc(engine: ^VulkanEngine, cmd: vk.CommandBuffer) {
	...
	vk.CmdDrawIndirect(cmd, engine.draw_cmds_buffer.buffer, 0, 1, 0) // stride is 0 since drawCount is 1
...
draw :: proc(engine: ^VulkanEngine) {
	...
	exec_voxel_compute(engine, cmd)

	util_transition_image(cmd, engine.draw_image.image, .GENERAL, .COLOR_ATTACHMENT_OPTIMAL)
	util_transition_image(cmd, engine.depth_image.image, .UNDEFINED, .DEPTH_ATTACHMENT_OPTIMAL);
