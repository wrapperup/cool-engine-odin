...
PrefilteredCubeMapPass :: struct {
	...
	prefilter_image_mapped_buffer: gfx.GPUBuffer,
	width:                         u32,
	height:                        u32,
}
...
create_prefiltered_cubemap_pipeline :: proc(filename: cstring, out_width, out_height: u32) -> PrefilteredCubeMapPass {
	...
	w, h: u32
	pass.cube_image = gfx.load_image_from_file(filename, .D2, .CUBE, &w, &h)
	...
	pass.prefilter_image_mapped_buffer = gfx.create_buffer(vk.DeviceSize(size), {.TRANSFER_DST}, .GPU_TO_CPU)
