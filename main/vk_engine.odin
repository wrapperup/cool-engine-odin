...
game_state := GameState {
	camera_pos     = {-11.25, 9.25, 12.73},
	camera_rot     = {-0.43, 0.615},
	sun_pos        = {12.0, 15.0, 10.0},
	camera_fov_deg = 45.0,
	sun_color      = {1.0, 1.0, 1.0},
	sky_color      = {0.4, 0.35, 0.55},
	bias           = 0.0012,
}
...
init_descriptors :: proc(engine: ^VulkanEngine) {
	...
	push_deletion_queue(&engine.main_deletion_queue, engine.global_descriptor_allocator.pool)
	...
	engine.mesh_descriptors = allocate_pool(
...
update_imgui :: proc(engine: ^VulkanEngine) {
	...
	if game_state.rotating_camera {
		game_state.camera_rot += {f32(pitch_delta), f32(yaw_delta)}
	}

	if game_state.rotating_camera != wants_rotate_camera {
		if wants_rotate_camera {
	...
	game_state.rotating_camera = wants_rotate_camera

	if (im.Begin("Camera")) {
		im.InputFloat3("pos", &game_state.camera_pos)
		im.InputFloat2("pitch yaw", &game_state.camera_rot)
		im.InputFloat("fov", cast(^f32)(&game_state.camera_fov_deg))
		items := [len(ViewState)]cstring{"SceneColor", "SceneDepth", "SunShadowDepth"}
		im.ComboChar("view", cast(^i32)(&game_state.view_state), raw_data(&items), len(items))
	}
	im.End()

	if (im.Begin("Sky")) {
		im.InputFloat3("pos", cast(^[3]f32)(&game_state.sun_pos))
		im.InputFloat3("target", cast(^[3]f32)(&game_state.sun_target))
		im.InputFloat3("sun_color", cast(^[3]f32)(&game_state.sun_color))
		im.InputFloat3("sky_color", cast(^[3]f32)(&game_state.sky_color))
		im.InputFloat("bias", cast(^f32)(&game_state.bias))
	}
	im.End()
...
update_game_state :: proc(engine: ^VulkanEngine) {
	{
		pitch := linalg.quaternion_angle_axis(game_state.camera_rot.x, [3]f32{1, 0, 0})
		yaw := linalg.quaternion_angle_axis(game_state.camera_rot.y, [3]f32{0, -1, 0})
		rotation := linalg.mul(yaw, pitch)

		forward := linalg.vector_normalize(linalg.quaternion_mul_vector3(rotation, [3]f32{0, 0, -1}))
		...
		if key_w {
			game_state.camera_vel += forward * 20 * engine.delta_time
		}
		if key_a {
			game_state.camera_vel += right * -20 * engine.delta_time
		}
		if key_s {
			game_state.camera_vel += forward * -20 * engine.delta_time
		}
		...
		if key_space {
			game_state.camera_vel += {0, 1, 0} * 20 * engine.delta_time
		}
		if key_shift {
			game_state.camera_vel += {0, -1, 0} * 20 * engine.delta_time
		}

		game_state.camera_pos += game_state.camera_vel * engine.delta_time
		if linalg.length(game_state.camera_vel) > 0.0 {
			friction: f32 = 0.1
			game_state.camera_vel = linalg.lerp(game_state.camera_vel, 0.0, 1-math.pow_f32(friction, engine.delta_time))
		}
	}
}
...
update_buffers :: proc(engine: ^VulkanEngine) {
	...
	// Camera matrices
	{
		...
		translation := linalg.matrix4_translate(game_state.camera_pos)

		pitch := linalg.quaternion_angle_axis(game_state.camera_rot.x, [3]f32{1, 0, 0})
		yaw := linalg.quaternion_angle_axis(game_state.camera_rot.y, [3]f32{0, -1, 0})
		rotation := linalg.matrix4_from_quaternion(linalg.mul(yaw, pitch))
	...
	// Global sun matrices
	{
		sun_view_matrix := linalg.matrix4_look_at_f32(game_state.sun_pos, game_state.sun_target, {0.0, 1.0, 0.0})
	...
	global_uniform_data.sun_color = game_state.sun_color
	global_uniform_data.sky_color = game_state.sky_color
	global_uniform_data.bias = game_state.bias

	global_uniform_data.camera_pos = hlsl.float3(game_state.camera_pos)
	global_uniform_data.sun_pos = hlsl.float3(game_state.sun_pos)
...
draw :: proc(engine: ^VulkanEngine) {
	...
	// Begin shadow pass
	util_transition_image(cmd, engine.shadow_depth_image.image, .UNDEFINED, .DEPTH_ATTACHMENT_OPTIMAL)
	draw_shadow_map(engine, cmd)

	util_transition_image(cmd, engine.draw_image.image, .UNDEFINED, .COLOR_ATTACHMENT_OPTIMAL)
	util_transition_image(cmd, engine.depth_image.image, .UNDEFINED, .DEPTH_ATTACHMENT_OPTIMAL)
	util_transition_image(cmd, engine.shadow_depth_image.image, .DEPTH_ATTACHMENT_OPTIMAL, .DEPTH_READ_ONLY_OPTIMAL)

	draw_geometry(engine, cmd)

	switch game_state.view_state {
	case .SceneColor:
